...
int wc::test::test_parser_if()
{
	return 1;
}


std::string test::getTypeAsString(wcTokenType _t)
{
	switch(_t)
	{
		case tt_null: 
...
std::string test::getTypeAsString(wcParseNodeType _t)
{
	switch(_t)
	{
...
std::string test::getTypeAsString(wcSymbolType _t)
{
	switch(_t)
	{
	case st_var:
...
std::string test::getTypeAsString(wcErrorCode t)
{
	switch(t)
	{
...
std::string test::getTypeAsString(vm::wcOpCode _t)
{
	switch(_t)
	{
...
std::string test::getTypeAsString(vm::wcValType _t)
{
	switch(_t)
	{
...
void test::printParser(wc::parse::wcParser* p)
{
	...
	for(auto it = p->ast.symTab->begin();it!=p->ast.symTab->end();++it)
	{
		...
		{
		case st_var:
			vars+=getTypeAsString(it->second.type) + " "+it->second.dataType+ "		  " +char(it->second.isConst+48)+" "+ it->second.fullIdent+" "+wc::util::wcitos(it->second.offset)+"\n"; 
			break;
		case st_namespace:
...
void test::printGen(wc::vm::wcByteCodeGen* bg)
{
	
	std::cout << "\n- - -\nwcByteCodeGen.istream\n- - -" << std::endl;
	int t=0;
