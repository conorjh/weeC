...
namespace parse{
		...
		int parseDecVar_Ident(wcParser* p_par, wcSymbol* p_type, wcSymbol*& p_ident, int* p_identX, int* p_identY, bool* p_setToConst);
		int parseDecVar_Exp(wcParser* p_par, wcSymbol*& p_ident, int*, int*);
...
void wc::parse::wcParser::startup()
{
	...
	//create tree
	ast.tree = new tree<wcParseNode>(wcParseNode(pn_head));
	pindex = ast.tree->begin();

	//create symbol and function table
	ast.funcTab = new std::unordered_map<string, wcFuncInfo>();
	ast.symTab = new std::unordered_map<string, wcSymbol>();
...
int wc::parse::parseDecVar_Ident(wcParser* p_par, wcSymbol* p_type, wcSymbol*& p_ident, int* p_identX, int* p_identY, bool* p_setToConst)
{
	switch (p_par->lexer->getToken()->type)
	{
	case tt_ident:
		...
		*p_identY = p_par->lexer->getToken()->y;	//for error reporting porpoises
		...
		p_par->addChild(wcParseNode(pn_ident, p_ident->fullIdent));
		break;
...
int wc::parse::parseDecVar_Exp(wcParser* p_par, wcSymbol*& p_ident, int *p_identX, int *p_identY)
{
	wcExpression ex;
...
int wc::parse::evalConstExp(wcParser* p_par, wcExpression ex)
{
